...
export default function(io: any, config: Config): void {
    io.on('connection', async (socket: any) => {
        ...
        const terminalName = socket.handshake.query.terminal;
        const { terminal: { host, port }, readonly } = user.terminals.filter((el: TerminalInfo) => el.terminal.name == terminalName)[0];
